<?php
namespace App\Controllers\Admin;
use App\Controllers\BaseController;
use CodeIgniter\HTTP\RequestInterface;
use CodeIgniter\HTTP\ResponseInterface;
use Psr\Log\LoggerInterface;


class Page_contact extends BaseController 
{
	public function initController(RequestInterface $request, ResponseInterface $response, LoggerInterface $logger) 
	{
        parent::initController($request, $response, $logger);
        $this->Model_common = new \App\Models\Admin\Model_common();
        $this->Model_page_contact = new \App\Models\Admin\Model_page_contact();
    }

    public function index()
	{
		$error = '';
		$success = '';

		$data['setting'] = $this->Model_common->get_setting_data();
		$data['page_contact'] = $this->Model_page_contact->show();

		echo view('admin/view_header',$data);
		echo view('admin/view_page_contact',$data);
		echo view('admin/view_footer');
		
	}

	public function edit($id)
	{
		
    	$tot = $this->Model_page_contact->page_contact_check($id);
    	if(!$tot) {
    		redirect(base_url().'admin/page-contact');
        	exit;
    	}
       	
       	$data['setting'] = $this->Model_common->get_setting_data();
		$error = '';
		$success = '';


		if(isset($_POST['form1'])) 
		{

			if(PROJECT_MODE == 0) {
				$this->session->setFlashdata('error',PROJECT_NOTIFICATION);
				redirect($_SERVER['HTTP_REFERER']);
			}

			$valid = 1;

			$contact_heading = $this->request->getPost('contact_heading', true);
			$contact_subtitle = $this->request->getPost('contact_subtitle', true);
			$contact_intro = $this->request->getPost('contact_intro', true);
			$contact_hours = $this->request->getPost('contact_hours', true);
			$contact_website = $this->request->getPost('contact_website', true);
			$contact_address = $this->request->getPost('contact_address', true);
			$contact_email = $this->request->getPost('contact_email', true);
			$contact_phone = $this->request->getPost('contact_phone', true);
			$contact_map = $this->request->getPost('contact_map');
			$mt_contact = $this->request->getPost('mt_contact', true);
			$mk_contact = $this->request->getPost('mk_contact', true);
			$md_contact = $this->request->getPost('md_contact', true);

			$this->form_validation->set_rules('contact_heading', 'Heading', 'trim|required');

			if($this->form_validation->run() == FALSE) {
				$valid = 0;
                $error .= validation_errors();
            }

		    if($valid == 1) 
		    {
	    		$form_data = array(
					'contact_heading' => $contact_heading,
					'contact_subtitle' => $contact_subtitle,
					'contact_intro' => $contact_intro,
					'contact_hours' => $contact_hours,
					'contact_website' => $contact_website,
					'contact_address' => $contact_address,
					'contact_email' => $contact_email,
					'contact_phone' => $contact_phone,
					'contact_map' => $contact_map,
					'mt_contact' => $mt_contact,
					'mk_contact' => $mk_contact,
					'md_contact' => $md_contact
	            );
	            $this->Model_page_contact->update($id,$form_data);				
				
				$success = 'Contact Page information is updated successfully';
				$this->session->setFlashdata('success',$success);
				redirect(base_url().'admin/page-contact');
		    }
		    else
		    {
		    	$this->session->setFlashdata('error',$error);
				redirect(base_url().'admin/page-contact/edit'.$id);
		    }
           
		} else {
			$data['page_contact'] = $this->Model_page_contact->get_page_contact($id);
	       	echo view('admin/view_header',$data);
			echo view('admin/view_page_contact_edit',$data);
			echo view('admin/view_footer');
		}

	}
	
}
