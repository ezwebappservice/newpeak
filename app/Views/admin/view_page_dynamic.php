<?php
if(!$this->session->userdata('id')) {
    redirect(base_url().'admin');
}
?>

<section class="content-header">
    <div class="content-header-left">
        <h1>View Dynamic Pages</h1>
    </div>
    <div class="content-header-right">
        <a href="<?php echo base_url(); ?>admin/page-dynamic/add" class="btn btn-primary btn-sm">Add New</a>
    </div>
</section>

<section class="content">
    <div class="row">
        <div class="col-md-12">
            <ul class="nav nav-pills" style="margin-bottom:15px;">
                <li class="<?php echo ($status_filter ?? 'active') === 'active' ? 'active' : ''; ?>">
                    <a href="<?php echo base_url(); ?>admin/page-dynamic?status=active">Active Pages</a>
                </li>
                <li class="<?php echo ($status_filter ?? '') === 'inactive' ? 'active' : ''; ?>">
                    <a href="<?php echo base_url(); ?>admin/page-dynamic?status=inactive">Inactive (Hidden)</a>
                </li>
                <li class="<?php echo ($status_filter ?? '') === 'all' ? 'active' : ''; ?>">
                    <a href="<?php echo base_url(); ?>admin/page-dynamic?status=all">All Pages</a>
                </li>
            </ul>

            <?php
            if(session()->getFlashdata('error')) {
                ?>
                <div class="callout callout-danger">
                    <p><?php echo session()->getFlashdata('error'); ?></p>
                </div>
                <?php
            }
            if(session()->getFlashdata('success')) {
                ?>
                <div class="callout callout-success">
                    <p><?php echo session()->getFlashdata('success'); ?></p>
                </div>
                <?php
            }
            ?>
            
            <div class="box box-info">
                <div class="box-body table-responsive">
                    <table id="example1" class="table table-bordered table-striped">
                        <thead>
                            <tr>
                                <th>SL</th>
                                <th>Banner</th>
                                <th>Name</th>
                                <th>Slug / URL</th>
                                <th>Status</th><th>Action</th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php
                            $i=0;
                            foreach ($page_dynamic as $row) {
                                $i++;
                                ?>
                                <tr>
                                    <td><?php echo $i; ?></td>
                                    <td><img src="<?php echo base_url(); ?>public/uploads/<?php echo $row['banner']; ?>" style="width:300px;"></td>
                                    <td><?php echo $row['name']; ?></td>
                                    <td>
                                        <code><?php echo $row['slug']; ?></code><br>
                                        <?php if(($row['status'] ?? 'Active') === 'Active'): ?>
                                        <a href="<?php echo base_url($row['slug']); ?>" target="_blank" rel="noopener"><?php echo base_url($row['slug']); ?></a>
                                        <?php else: ?>
                                        <span class="text-muted">Hidden on website</span>
                                        <?php endif; ?>
                                    </td>
                                    <td>
                                        <?php if(($row['status'] ?? 'Active') === 'Active'): ?>
                                        <span class="label label-success">Active</span>
                                        <?php else: ?>
                                        <span class="label label-default">Inactive</span>
                                        <?php endif; ?>
                                    </td><td>
                                        <?php if(($row['status'] ?? 'Active') === 'Active'): ?>
                                        <a href="<?php echo base_url($row['slug']); ?>" target="_blank" class="btn btn-success btn-sm btn-block">View</a>
                                        <?php endif; ?>
                                        <a href="<?php echo base_url(); ?>admin/page-dynamic/edit/<?php echo $row['id']; ?>" class="btn btn-primary btn-sm btn-block">Edit</a>
                                        <a href="<?php echo base_url(); ?>admin/page-dynamic/delete/<?php echo $row['id']; ?>" class="btn btn-danger btn-sm btn-block" onClick="return confirm('Are you sure?');">Delete</a>
                                    </td>
                                </tr>
                                <?php
                            }
                            ?>
                        </tbody>
                    </table>
                </div>
            </div>
        </div>
    </div>
</section>