...
<section class="content-header"><div class="content-header-left"><h1>Edit Sub Category</h1></div><div class="content-header-right"><a href="<?php echo base_url(); ?>admin/shop_sub_category" class="btn btn-primary btn-sm">View All</a></div></section>
...
<?php echo form_open_multipart(base_url().'admin/shop_sub_category/edit/'.$category['sub_category_id'],array('class'=>'form-horizontal')); ?>
<div class="box box-info"><div class="box-body">
<div class="form-group"><label class="col-sm-2 control-label">Parent Category <span>*</span></label><div class="col-sm-4"><select name="parent_category_id" class="form-control select2" required><?php foreach($parent_categories as $row): ?><option value="<?php echo $row['parent_category_id']; ?>" <?php if($category['parent_category_id']==$row['parent_category_id']) echo 'selected'; ?>><?php echo $row['category_name']; ?></option><?php endforeach; ?></select></div></div>
...
<div class="form-group"><label class="col-sm-2 control-label">Existing Image</label><div class="col-sm-9"><img src="<?php echo base_url(); ?>public/uploads/<?php echo $category['category_image']; ?>" style="width:150px;"></div></div>
<div class="form-group"><label class="col-sm-2 control-label">Image</label><div class="col-sm-9"><input type="file" name="category_image"></div></div>
